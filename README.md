...
Tiny HTTP service that takes `{videoId, start, end}` and streams back a trimmed MP4 of that exact segment. Used by the Lovable Cloud download-clip edge function so iPhone Safari + desktop both get a real file download without any client-side ffmpeg.
...
- `GET /healthz` → `{"ok": true}`
- `GET /clip?videoId=XXXX&start=12.5&end=37&title=my_clip`
...
# Clip (replace XXXX with a real YouTube video ID)
curl -H "x-worker-token: mysecret" \
  "http://localhost:3000/clip?videoId=XXXX&start=10&end=40&title=test" \
  -o test.mp4
```
